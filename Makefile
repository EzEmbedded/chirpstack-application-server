...
API_VERSION := $(shell go list -m -f '{{ .Version }}' github.com.cnpmjs.org/brocaar/chirpstack-api/go/v3 | awk '{n=split($$0, a, "-"); print a[n]}')
...
proto:
	...
	@git clone https://github.com.cnpmjs.org/brocaar/chirpstack-api.git /tmp/chirpstack-api
...
dev-requirements:
	...
	go install github.com.cnpmjs.org/goreleaser/goreleaser
	go install github.com.cnpmjs.org/goreleaser/nfpm
	go install github.com.cnpmjs.org/golang/protobuf/protoc-gen-go
	go install github.com.cnpmjs.org/golang-migrate/migrate/v4/cmd/migrate
